...
// AES-256-GCM encryption using Web Crypto (available in Deno)
async function encryptPassword(plaintext: string, keyStr: string): Promise<string> {
  ...
  // base64url encode for safe DB storage
...
serve(async (req) => {
  ...
  try {
    ...
    if (!encryptionKey) {
      return jsonResponse({ error: "WP_ENCRYPTION_KEY not configured" }, 500);
    }
    ...
    // Only update encrypted password if a new one was provided
    if (wpAppPassword && wpAppPassword.trim()) {
      updatePayload.wp_app_password_enc = await encryptPassword(wpAppPassword.trim(), encryptionKey);
      updatePayload.wp_app_password = null; // clear plaintext
    }
    ...
    console.error("[save-wordpress-credentials] Error:", e);
