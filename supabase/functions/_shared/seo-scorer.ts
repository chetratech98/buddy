/**
 * Deno port of src/lib/seo-scorer.ts — kept algorithmically identical so a
 * score computed here at generation time matches what the frontend (Create/
 * EditPost) would compute later for the same content. Update both together.
 *
...
function extractKeywordTerms(keywords: string[]): string[] {
  return keywords.flatMap((k) => k.split(",")).map((k) => k.trim().toLowerCase()).filter(Boolean);
}
